...
{
	SIZE_T read = 0;
	NTSTATUS res = ReadProcessMemory(process, (PVOID*)addr, buf, size, &read);
	return !res;
}
...
BOOL pe_load_nt_header(HANDLE hProc, DWORD64 base, IMAGE_NT_HEADERS* nth)
{
	...
	NTSTATUS res = ReadProcessMemory(hProc, (PVOID*)(DWORD_PTR)base, &dos, sizeof(dos), NULL);
	NTSTATUS res2 = ReadProcessMemory(hProc, (PVOID*)(DWORD_PTR)(base + dos.e_lfanew), nth, sizeof(*nth), NULL);
